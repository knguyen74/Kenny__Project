package com.example.kenny.kennyproject.audio;
...
public class MusicService extends Service {
	...
	public static final int SEEKTO_MSG = 5;// start from particulartime

	public static final String MusicServiceStatus = "cs.gsu.edu"; // Broadcast actionAction
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		UtilLog.logD("Music","start of onStartCommand");
		if (intent == null) return super.onStartCommand(intent, flags, startId);
			...
			switch (msg) {
				...
				case PLAY_MSG:
					if (url != null) {
						UtilLog.logD("Music","b4 prepare play in onStartCommmand");
						preparePlay();
					} else {
	...
	private void preparePlay() {
		if (lastURL == null || lastURL.equals(url)) {
			...
			try {
				UtilLog.logD("Music","b4 notifyLoading");
				notifyLoading();
				mediaPlayer.reset();
